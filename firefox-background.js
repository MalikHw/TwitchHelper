...
browser.storage.local.get(['checkInterval']).then((result) => {
...
async function checkStreams() {
  try {
    const result = await browser.storage.local.get(['streamers']);
...
browser.notifications.onClicked.addListener((notificationId) => {
  browser.storage.local.get([`notification_${notificationId}`]).then((result) => {
    const streamer = result[`notification_${notificationId}`];
    if (streamer) {
      browser.tabs.create({
        url: `https://www.twitch.tv/${streamer}`
      });
      browser.storage.local.remove([`notification_${notificationId}`]);
    }
  });
});
